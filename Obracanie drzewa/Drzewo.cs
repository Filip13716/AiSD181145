...
namespace Binarne_drzewo_poszukiwań
{
    internal class Drzewo
    {
        ...
        public void obrotPrawo(Węzeł y)
        {
            Węzeł x = y.dzieckoLewe;
            if (x == null) return;

            Węzeł B = x.dzieckoPrawe;

            x.rodzic = y.rodzic;

            if (y.rodzic == null)
                root = x;
            else if (y.rodzic.dzieckoLewe == y)
                y.rodzic.dzieckoLewe = x;
            else
                y.rodzic.dzieckoPrawe = x;
        ...
        public void obrotLewo(Węzeł y)
        {
            Węzeł x = y.dzieckoPrawe;
            if (x == null) return;

            Węzeł B = x.dzieckoLewe;
        ...
        public Węzeł Search(int v)
        {
            ...
            while (true)
            {
                ...
                if (ptr.dzieckoLewe == null && ptr.dzieckoPrawe == null) throw new Exception("Wartości nie ma w drzewie");

                if (v < ptr.value) ptr = ptr.dzieckoLewe;
                if (v > ptr.value) ptr = ptr.dzieckoPrawe;
            }
        }
    }
}
